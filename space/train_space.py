...
from space.datasets.build import build_dataloader
from space.train_utils import build_optimizer, build_lr_scheduler, get_priors, get_weights
...
def main():
    ...
    set_random_seed(cfg.RNG_SEED)
    train_dataloader = build_dataloader(cfg, True, start_iter)
    ...
    if do_validation:
        val_dataloader = build_dataloader(cfg, training=False)
    ...
    for iteration, data_batch in enumerate(train_dataloader, start_iter):
        ...
        # get current priors
        prior_batch = {k: (v(iteration) if callable(v) else v) for k, v in prior_dict.items()}
        prior_batch = {k: torch.tensor(v) for k, v in prior_batch.items()}
        data_batch.update(prior_batch)

        # copy data from cpu to gpu
        data_batch = {k: v.cuda(non_blocking=True) for k, v in data_batch.items()}

        # get current weights
        weight_batch = {k: (v(iteration) if callable(v) else v) for k, v in weight_dict.items()}

        # forward
        pd_dict = model(data_batch, **weight_batch)

        # update losses
        loss_dict = model.compute_losses(pd_dict, data_batch, **weight_batch)
        ...
        if summary_writer is not None and (summary_period > 0 and cur_iter % summary_period == 0):
            ...
            # summarize weights
            for name, value in weight_batch.items():
                summary_writer.add_scalar('train/' + name, value, global_step=cur_iter)

            # summarize priors
            for name, value in prior_batch.items():
                if value.numel() == 1:
                    summary_writer.add_scalar('train/' + name, value.item(), global_step=cur_iter)

            model.summarize(pd_dict, data_batch, summary_writer, cur_iter, prefix='train/')
        ...
        if do_validation and (cur_iter % val_period == 0 or cur_iter == max_iter):
            ...
            if summary_writer is not None:
                ...
                # visualization
                model.summarize(pd_dict, data_batch, summary_writer, cur_iter, prefix='val/')
